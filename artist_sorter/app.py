...
class ArtistSorterApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.download_btn = ttk.Button(
            outer, text="DB 갱신", command=self._download_db
        )
        ...
        ttk.Radiobutton(
            options, text="이동", variable=self.mode_var, value="move"
        ).pack(side="left", padx=(8, 4))
        ttk.Radiobutton(
            options, text="복사", variable=self.mode_var, value="copy"
        ...
        self.preview_btn = ttk.Button(
            actions, text="1. 미리보기", command=self._preview
        )
        self.preview_btn.pack(side="left")
        self.run_btn = ttk.Button(
            actions, text="2. 정리 실행", command=self._run_plan, state="disabled"
        )
        ...
        for col in columns:
            ...
            self.tree.column(
                ...
                width=widths[col],
                minwidth=70,
                stretch=col in {"source", "artists", "title", "destination"},
            )
    ...
    def _preview(self) -> None:
        ...
        threading.Thread(
            ...
            args=(source, output, db_path),
            daemon=True,
        ).start()

    def _preview_worker(
        self, source: Path, output: Path, db_path: Path
    ) -> None:
        try:
            plan = make_plan(
                source, output, db_path, self.artist_strategy_var.get()
            )
    ...
    def _download_db(self) -> None:
        ...
        if not url:
            messagebox.showerror(
                "URL 필요", "Pi의 DB 다운로드 URL을 입력해주세요."
            )
            return
            ...
            while True:
                ...
                if kind == "preview_done":
                    ...
                    matched = sum(
                        1
                        for x in self.plan
                        if x.gallery_id is not None and x.artists
                    )
                    self._set_busy(
                        False,
                        f"총 {len(self.plan)}개 / "
                        f"작가 매칭 {matched}개 / 실행 가능 {ready}개",
                    )
                    ...
                    self._set_busy(
                        False,
                        f"완료 {completed}개 / 건너뜀 {skipped}개 / "
                        f"실패 {len(errors)}개",
                    )
                    detail = "\n".join(errors[:10])
                    if len(errors) > 10:
