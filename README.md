...
- 작가 미상은 화면에서 \`N/A\`, 실제 Windows 폴더는 \`기타\`로 분류
  - 작가 미상과 DB 미매칭 항목을 한곳에 모으기 위해 \`기타\`를 사용합니다.
- 작품 번호는 잡히지만 Violet DB에 없는 항목도 \`기타\`로 분류
  - 표/통계에는 계속 \`DB 미매칭\`으로 표시해 작가 미상과 구분합니다.
...
파일명에 작품명 추가 옵션을 켜면 파일은 `4192094 (작품명).zip` 형식으로 저장됩니다. DB 미매칭, 제목 없음, 폴더형 작품은 기존 이름을 유지하며 Windows 파일명 금지문자는 `_`로 치환합니다. NAS/SMB 호환성을 위해 이모지/variation selector는 제거하고 최종 파일명은 UTF-8 기준 200바이트 이하로 제한합니다. 제목이 이모지만 남으면 `4192094.zip`처럼 ID-only 이름으로 저장합니다.
